...
export const useRegisterCat = () => {
  ...
  const handleRequestCurrentLocation = () => {
    getCurrentPosition()
      ...
      .catch(() => {
        setGeoError(
          t("cat.geoError", "Impossible d'obtenir votre position actuelle")
        );
      });
  };
  ...
  const handleChange = (e) => {
      ...
      if (value) {
        ...
        if (tempFormData.reportDate) {
          ...
          if (selectedDate > reportDate) {
            newValidationErrors.dateComparison = t(
              "cat.errorDateComparison",
              "La date de signalement ne peut pas être antérieure à la date de naissance"
            );
          } else {
    ...
    if (name === "reportDate") {
      if (value) {
        ...
        if (selectedDate > today) {
          newValidationErrors.reportDate = t(
            "cat.errorFutureReportDate",
            "La date de signalement ne peut pas être dans le futur"
          );
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    const name =
      formData.name.trim() === "" ? t("cat.unknown", "Inconnu") : formData.name;
    setFormData((prev) => ({
      ...prev,
      name: name,
    }));
    ...
    formDataWithPayload.append(
      "catData",
      new Blob([JSON.stringify(catStatus)], {
