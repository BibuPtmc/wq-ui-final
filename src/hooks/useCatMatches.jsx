...
const useCatMatches = (type) => {
   ...
   */
  const fetchMatchesForCat = useCallback(async (catId) => {
   ...
   */
  const fetchMatchesForCats = useCallback(async (cats) => {
   ...
   * @param {number} catId - ID du chat
   */
  const clearCacheForCat = useCallback((catId) => {
    const cacheKey = `matches_${type}_${catId}`;
