...
const LostCatsMap = ({ noLostCatsMessage }) => {
  ...
  const markers = lostCats
    .map((catStatus) => {
      ...
      const catImage =
        catStatus.cat.imageUrls && catStatus.cat.imageUrls.length > 0
      ...
      return {
        ...
        <div class="cat-popup">
          ...
          catStatus.cat.name || "Chat sans nom"
        }" class="cat-popup-img" onerror="this.src='/noImageCat.png'; this.onerror=null;">
          <h5 class="cat-popup-title">${
            catStatus.cat.name || "Chat sans nom"
          }</h5>
          <p class="cat-popup-date">Signalé perdu le: ${reportDate}</p>
          
          <p class="cat-popup-info"><strong>Race:</strong> ${
            formatValue(catStatus.cat.breed) || "Non spécifiée"
          }</p>
          <p class="cat-popup-info"><strong>Genre:</strong> ${
            formatValue(catStatus.cat.gender) || "Non spécifié"
          }</p>
          <p class="cat-popup-info"><strong>Couleur:</strong> ${
            formatValue(catStatus.cat.color) || "Non spécifiée"
          }</p>
          <p class="cat-popup-info"><strong>Couleur des yeux:</strong> ${
            formatValue(catStatus.cat.eyeColor) || "Non spécifiée"
          }</p>
          ${
            catStatus.cat.dateOfBirth
              ? `<p class="cat-popup-info"><strong>Âge:</strong> ${calculateAge(
                  catStatus.cat.dateOfBirth
                )} ans</p>`
              : ""
          }
        ...
        <Card.Body className="text-center py-5">
          <Spinner animation="border" role="status" variant="primary">
            <span className="visually-hidden">
              {t("map.loadingLostCats", "Chargement...")}
            </span>
          </Spinner>
          <p className="mt-3">
            {t("map.loadingLostCats", "Chargement des chats perdus...")}
          </p>
        </Card.Body>
      </Card>
    );
      ...
      <Card.Body>
        <div className="mb-3">
          ...
          >
            ...
            {isLocating
              ...
              : t("map.centerOnMyPosition", "Centrer sur ma position")}
          </Button>
        </div>
        <MapLocation
        ...
        >
          <div className="d-flex align-items-center">
            ...
            <span>
              {t(
                "map.clickMarkerDetails",
                "Cliquez sur les marqueurs pour voir les détails des chats perdus"
              )}
            </span>
          </div>
        </Alert>
